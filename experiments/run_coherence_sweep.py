...
def main():
    ...
    coherence_strengths = [0.25, 0.35, 0.50]

    # Same phase-transition grid as before.
    m_values = [96, 128, 160]
    ...
    for coherence_strength in coherence_strengths:
        for m in m_values:
            for k_ratio in k_over_m_values:
                ...
                print("=" * 80)
                print(
                    f"coherence={coherence_strength}, "
                    f"n={n}, m={m}, k={k}, k/m={k/m:.3f}"
                )
    ...
    out_path = ROOT / "results" / "coherence_sweep_results.csv"
    df.to_csv(out_path, index=False)

    print(f"Saved coherence sweep results to {out_path}")
    print()
    print(
        df.groupby(["coherence_strength", "m", "k", "method"])[["nrmse", "f1"]]
        .mean()
        .round(4)
    )
